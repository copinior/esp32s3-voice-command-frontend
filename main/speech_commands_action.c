...
static const char *TAG = "SPEECH_ACTION";

/* Set to 1 if you want to play prompt audio again. */
#define APP_PROMPT_PLAYBACK_ENABLE   (0)
...
{
    if (APP_PROMPT_PLAYBACK_ENABLE) {
        esp_audio_play((int16_t *)(playlist[0].data), playlist[0].length, portMAX_DELAY);
    } else {
        ESP_LOGI(TAG, "wake prompt playback disabled");
    }
}
...
void speech_commands_action(int command_id)
{
    if (APP_PROMPT_PLAYBACK_ENABLE) {
        ...
        ESP_LOGI(TAG, "command playback disabled, cmd=%d", command_id);
    }
}
